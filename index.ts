...
// import * as IDK from 'three.terrain.js';
  ...
  for (var i = 0; i < geometry.attributes.position.array.length; i += 9) {
    ...
    if (spreadIsNumber) {
      ...
      if (rv < options.spread) {
        ...
        place =
          THREE.Terrain.EaseInOut((rv - options.spread) * spreadRange) *
            options.spread >
          Math.random();
      }
    } else {
...
const sphere = new THREE.Mesh(new THREE.SphereGeometry(1.2, 80, 80));

// Make sure the .matrix of each mesh is current
box.updateMatrix();
sphere.updateMatrix();

const floorGeo = new THREE.BoxGeometry(500, 500, 500);
floorGeo.applyMatrix4(new THREE.Matrix4().makeTranslation(0, 0, 250));
...
const ambient = new THREE.AmbientLight(0x999999); // soft white light
scene.add(ambient);
...
// console.log('t', terrainScene);

const terrainMesh: Mesh = terrainScene.children[0] as Mesh;
...
const foo = CSG.intersect(floor, terrainMesh);
foo.updateMatrix();
foo.rotateX(-Math.PI / 2);
foo.updateMatrixWorld();
foo.castShadow = true;
foo.receiveShadow = true;
foo.material = new THREE.MeshPhongMaterial({ color: 0xffffff });
scene.add(foo);

const xSpacing = 10;
const zSpacing = 10;

function getTree(yNorm) {
  const trunkHeight = 2;
  const trunkGeo = new THREE.CylinderGeometry(1, 1, trunkHeight, 20);
  trunkGeo.applyMatrix4(
    new THREE.Matrix4().makeTranslation(0, trunkHeight / 2, 0)
  );
  ...
  const topColors = [0xffeb3b, 0xffc107, 0xff9800, 0xff5722, 0xf44336];
  const color = topColors[Math.floor(Math.random() * topColors.length)];

  const topHeight = 6;
  const topGeo = new THREE.CylinderGeometry(0.01, 2, topHeight, 100);
  topGeo.applyMatrix4(
    new THREE.Matrix4().makeTranslation(0, trunkHeight + topHeight / 2, 0)
  );
  const top = new THREE.Mesh(topGeo, new THREE.MeshStandardMaterial({ color }));
  top.castShadow = true;
  ...
  group.add(trunk);
  group.add(top);
  const scale = yNorm * yNorm * yNorm * 1.5;
...
const boo = new THREE.Mesh(
  new THREE.BoxGeometry(250, 10, 250),
  new THREE.MeshStandardMaterial({ color: 0xff00ff })
);
scene.add(boo);

while (currTest < numTests) {
  const x = Math.floor(Math.random() * (maxX + Math.abs(minX)) + minX);
  const z = Math.floor(Math.random() * (maxZ + Math.abs(minZ)) + minZ);
  ...
  const intersection = raycaster.intersectObject(boo, false)[0];
  ...
  if (intersection) {
    ...
    normalMatrix.getNormalMatrix(foo.matrixWorld);
    var v1 = new THREE.Vector3();
    const normal = intersection.face.normal;
    v1.copy(normal).applyMatrix3(normalMatrix).normalize();

    // console.log(v1);
    if (v1.y > 0.9) {
      var quaternion = new THREE.Quaternion(); // create one and reuse it
      quaternion.setFromUnitVectors(new THREE.Vector3(0, 1, 0), v1);

      const tree = getTree(v1.y);

      tree.applyQuaternion(quaternion);

      tree.position.x = intersection.point.x;
      tree.position.y = intersection.point.y;
      tree.position.z = intersection.point.z;
...
// pointLight.shadow.camera.left = -250;
// pointLight.shadow.camera.top = 250;
// pointLight.shadow.camera.right = 250;
// pointLight.shadow.camera.bottom = -250;
...
renderer.setClearColor(0xe9cbfa);
